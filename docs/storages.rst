...
Your storage only needs to inherit from ``PipelineMixin`` and
``ManifestFilesMixin`` or ``CachedFilesMixin``. In Django 1.7+ you should 
use `ManifestFilesMixin <https://docs.djangoproject.com/en/1.7/ref/contrib/staticfiles/#manifeststaticfilesstorage>`_
which eliminates network queries to an external cache like memcached, 
unless you don't have access to the local filesystem in which case you
should use ``CachedFilesMixin``. ::
